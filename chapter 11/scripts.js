...
var request = (function () {
  function error(device) {
    return (req, resolve, reject) =>
      reject(new Error(device + ': malformed request'));
  }
  ...
  class Screens {
    constructor() {
      ...
      for (let i = 0; i < 9; i++) {
        ...
        screen.style.cssText =
          'border: 1px solid #222; background: black; position: absolute; width: 33.3%; height: 33.3%; left: ' +
          col * 33.3 +
          '%; top: ' +
          row * 33.3 +
          '%';
    ...
    update(n, data) {
      this.getParent();
      if (!this.parent.ownerDocument.body.contains(this.dom))
        this.parent.appendChild(this.dom);

      let canvas = this.screens[n],
        cx = canvas.getContext('2d');
      cx.clearRect(0, 0, canvas.width, canvas.height);
      let gapX = (canvas.width * 0.4) / 51,
        sizeX = (canvas.width * 0.6) / 50,
        skipX = gapX + sizeX;
      let gapY = (canvas.height * 0.4) / 31,
        sizeY = (canvas.height * 0.6) / 30,
        skipY = gapY + sizeY;
